...
from .service_adapter import UiSettings, ingest_file, inspect_qdrant, semantic_search
...
class DebugUiApp:
    ...
    def _build_layout(self) -> None:
        ...
        settings_frame = ttk.LabelFrame(container, text="公共调试配置", padding=10)
        ...
        status_label.grid(row=3, column=0, sticky="w", pady=(8, 4))

        log_frame = ttk.LabelFrame(container, text="调试日志", padding=8)
    ...
    def _build_ingest_tab(self) -> None:
        ...
        ttk.Checkbutton(action_row, text="执行写库", variable=self.ingest_write_var).pack(
            side=tk.LEFT
        )
    ...
    def on_ingest(self) -> None:
        ...
        write_to_store = self.ingest_write_var.get()
        try:
            self.log("开始执行单文件入库")
